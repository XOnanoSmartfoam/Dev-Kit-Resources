...
This program is used for XO-Balance board calibration as well as actual use. The
BLE needs to be turned on during calibration to simulate the BLE module interaction
with ADC2.
************************************************************************************/
#include <BLEDevice.h>
#include <BLEServer.h>
...
#define DEVICE_NAME "XOBalanceC"
/*************************************
User Specific Edit Section
*************************************/

// Replace a, b, c, d, and VSS with load calibration coefficients below
float a[numOfADCs] {};
float b[numOfADCs] {};
float c[numOfADCs] {};
float d[numOfADCs] {};
float VSS[numOfADCs] {};
float balanceCoefficient = 1.0; // For balancing with board #.
...
void update_voltages()
{
  ...
  for(int i = 0; i < numOfADCs; i++)
  {
    ...
    Serial.print(Vrms[i]); // Voltage level check. Note that ADC2's voltage readings shift with the use of the BLE module.
    if (i < numOfADCs - 1)
    {
      Serial.print(",");
    }
...
pServer->getAdvertising()->setScanResponse(true); // not sure if I need this.
...
void print_txValue()
{
  for(int i = 0; i < numOfBLEPackets; i++)
  {
    Serial.print(txValue[i]);
    if(i%numOfPacketsPerADCPin == numOfPacketsPerADCPin-1 && i < numOfBLEPackets-1)
    {
      Serial.print(",");
    }
  }
